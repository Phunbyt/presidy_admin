    ...
    <div className="p-4 sm:p-6 md:p-8 space-y-6" style={{ background: t.bg, minHeight: '100%', transition: 'background 0.2s' }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 sm:px-5 py-2 sm:py-2.5 rounded-lg transition-all font-semibold text-sm flex-shrink-0 whitespace-nowrap"
          style={{ background: 'linear-gradient(135deg, #D4A843, #B8882A)', color: '#000' }}
        >
          <Zap size={14} />
          <span className="hidden sm:inline">Auto Pay All Moderators</span>
          <span className="sm:hidden">Auto Pay</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
        {[
          ...
          return (
            <div key={stat.label} className="rounded-lg sm:rounded-xl p-4 sm:p-5" style={{ background: t.surface, border: `1px solid ${t.border}`, transition: 'background 0.2s' }}>
              ...
              <p className="text-lg sm:text-xl font-bold" style={{ fontFamily: 'Syne, sans-serif', color: t.text }}>{stat.value}</p>
            </div>
          );
        })}
      </div>
      ...
      <div className="rounded-lg sm:rounded-xl overflow-hidden" style={{ background: t.surface, border: `1px solid ${t.border}`, transition: 'background 0.2s' }}>
        {/* Toolbar */}
        <div className="px-3 sm:px-5 py-3 sm:py-4 flex flex-col sm:flex-row gap-3 sm:gap-3 items-start sm:items-center" style={{ borderBottom: `1px solid ${t.borderSub}` }}>
          ...
          {selectedModerators.length > 0 && (
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 ml-auto w-full sm:w-auto">
            ...
            <tbody>
              {filtered.map((mod, index) => (
                ...
                >
                  ...
                  <td className="px-3 sm:px-5 py-2 sm:py-4 text-xs sm:text-sm font-medium whitespace-nowrap" style={{ color: t.textSub }}>
                    {mod.users.toLocaleString()}
                  </td>
                  <td className="px-3 sm:px-5 py-2 sm:py-4 whitespace-nowrap">
                    <span className="text-xs sm:text-sm font-semibold" style={{ color: '#D4A843' }}>{fmt(mod.pendingAmount)}</span>
                  </td>
                  <td className="px-3 sm:px-5 py-2 sm:py-4">
                    <div className="flex items-center gap-1.5">
                      <Clock size={11} style={{ color: t.textFaint }} />
                      <span className="text-xs whitespace-nowrap" style={{ color: t.textMuted }}>{mod.lastPaid}</span>
                    </div>
                  </td>
                  <td className="px-3 sm:px-5 py-2 sm:py-4">
                    <span
                  ...
                  <td className="px-3 sm:px-5 py-2 sm:py-4">
                    <button
                      ...
                      className="px-2 sm:px-3 py-1.5 rounded-md text-xs font-semibold transition-all disabled:opacity-50 whitespace-nowrap"
